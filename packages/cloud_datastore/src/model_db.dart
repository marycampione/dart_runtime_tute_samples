...
class ModelDB {
   ...
   */
  Key fromDatastoreKey(rawDatastore.Key datastoreKey) {
   ...
   */
  rawDatastore.Key toDatastoreKey(Key dbKey) {
    List<rawDatastore.KeyElement> elements = [];
    var currentKey = dbKey;
    while (currentKey._parent is! Partition) {
      var id = dbKey.id;

      var modelDescription = modelDescriptionForType(dbKey.type);
      var idProperty =
      ...
      elements.add(new rawDatastore.KeyElement(kind, dbKey.id));
      currentKey = currentKey.parent;
    }
    Partition partition = currentKey._parent;
    return new rawDatastore.Key(
        elements.reversed.toList(),
        partition: new rawDatastore.Partition(partition.namespace));
  }

  /**
   * Converts a [Model] instance to a [datastore.Entity].
   */
  rawDatastore.Entity toDatastoreEntity(Model model) {
    try {
   ...
   */
  Model fromDatastoreEntity(rawDatastore.Entity entity) {
