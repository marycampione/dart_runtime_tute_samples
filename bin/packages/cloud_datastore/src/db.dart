...
class Transaction {
  ...
  final rawDatastore.Transaction _datastoreTransaction;
...
class Query {
  final _relationMapping = const <String, rawDatastore.FilterRelation> {
    '<': rawDatastore.FilterRelation.LessThan,
    '<=': rawDatastore.FilterRelation.LessThanOrEqual,
    '>': rawDatastore.FilterRelation.GreatherThan,
    '>=': rawDatastore.FilterRelation.GreatherThanOrEqual,
    '=': rawDatastore.FilterRelation.Equal,
    'IN': rawDatastore.FilterRelation.In,
  };

  final DatastoreDB _db;
  final rawDatastore.Transaction _transaction;
  final String _kind;
  ...
  final List<rawDatastore.Filter> _filters = [];
  final List<rawDatastore.Order> _orders = [];
  int _offset;
  int _limit;

  Query(DatastoreDB dbImpl, ModelDescription modelDescription,
        {Partition partition, Key ancestorKey,
         rawDatastore.Transaction datastoreTransaction})
  ...
  void filter(String filterString, Object comparisonObject) {
    ...
    _filters.add(new rawDatastore.Filter(
  ...
  void order(String orderString) {
    ...
    if (orderString.startsWith('-')) {
      _orders.add(new rawDatastore.Order(
          rawDatastore.OrderDirection.Decending,
          _convertToDatastoreName(orderString.substring(1))));
    } else {
      _orders.add(new rawDatastore.Order(
          rawDatastore.OrderDirection.Ascending,
  ...
  Future<List<Model>> run() {
    ...
    var query = new rawDatastore.Query(
    ...
    var partition;
    if (_partition != null) {
      partition = new rawDatastore.Partition(_partition.namespace);
    }

    return _db.datastore.query(
        query, transaction: _transaction, partition: partition)
        .then((List<rawDatastore.Entity> entities) {
...
class DatastoreDB {
  final rawDatastore.RawDataStore datastore;
  ModelDB modelDB;
  Partition _defaultPartition;

  DatastoreDB(this.datastore) {
    modelDB = new ModelDB();
  ...
  Future<Transaction> beginTransaction({bool crossEntityGroup: false}) {
    return datastore.beginTransaction(crossEntityGroup: crossEntityGroup)
        .then((rawDatastore.Transaction transaction) {
...
Future _commitHelper(DatastoreDB db,
  ...
  return db.datastore.commit(inserts: entityInserts,
      ...
      .then((rawDatastore.CommitResult result) {
